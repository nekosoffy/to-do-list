...
const taskDisplay = () => {
    ...
    tasks.forEach((task, index) => {
      ...
      for (const element of task) {
        if (
          ...
          Object.keys(element)[0] === 'Description:' &&
          Object.values(element)[0] !== ''
        ) {
          const li = document.createElement('li');
          ...
          if (Object.values(element)[0]) {
            ...
            create(
              ...
              '',
              'item-value',
              ` ${format(Object.values(element)[0], 'yyyy/MM/dd')}`,
            );
            ul.appendChild(li);
          }
        } else if (
          nextElements.includes(Object.keys(element)[0]) &&
          Object.values(element)[0] !== ''
        ) {
          const li = document.createElement('li');
          li.classList.add('task-li');
          ...
          for (const item of Object.values(element)[0]) {
            const li = document.createElement('li');
            li.classList.add('checklist-li');
            const checklistItemBtn = create(
              'button',
              li,
              '',
              'checklist-status',
  ...
  function handleNewChecklistItem() {
    if (itemInput.value !== '') {
      const text =
        itemInput.value.at(0).toUpperCase() + itemInput.value.slice(1);
      itemInput.value = '';
  ...
  function handleSubmit(event) {
    ...
    const checklistValues = Array.from(checklistItems).map(
